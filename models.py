...
def cie94(y_true, y_pred):
    ...
    normalizing_constant = batch_size * 384 * 384  # Height and Width are hardcoded, reading y_true.shape[1] and
    # y_true.shape[2] is problematic on different environments.
    for i in range(0, batch_size):
        ...
        loss += tf.keras.backend.sum(tf.keras.backend.sqrt(
            delta_Csqr / tf.keras.backend.square(1.0 + K_1 * C1) + delta_H_square / tf.keras.backend.square(
                1.0 + K_2 * C1)), axis=0)
